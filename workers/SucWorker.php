...
class SucWorker extends Worker
{
  ...
  public function run(){
	...
	elseif($result =='NotDetermined'){                                             //낙찰자를 못정한상태
		$post_data=[
			[ 
				'action'=>'smartsuit.ui.etnajs.pro.rfx.sp.OpenInfoDataDetailController',
				'data'=>[$this->id],
    ...
    elseif($result =='ReBidding'){                              //확인이 안됨
     // $data['nbidcomment'] = '유찰';
     // $data['bidproc'] = 'F';
     // $data['notinum'] = $this->alldata['no'];
     // $data['revision'] = $this->alldata['revision'];
    ...
    elseif($result =='NoticeChange'){                           //공고종료 재공고 나는것들
      return;
    }
	...
	//       print_r($min_arr);
		...
		elseif($result =='NotDetermined'){
			...
			$data['res']['officenm1'] = $a_arr[0]['officenm'];
			$data['res']['officeno1'] = $a_arr[0]['officeno'];
